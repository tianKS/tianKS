        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转。从a路由条状到b路由。这里是在加入购物车，进行路由跳转之前，发请求
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据,如果服务器数据没有回来，skuInfo这个空对象
    skuImageList() {
  ...
  methods: {
    ...
    //加入购物车
    async addShopcar() {
      //1.发请求---将产品加入到数据库，当前函数返回一个promise
      try {
        //成功
        ...
        //2.服务器存储成功---进行路由的跳转
        // 在路由跳转的时候还需要将产品的信息带给下一级的组件
        //下面这种手段路由跳转一级传递参数可以
        //简单的参数可以通过路由组件传递过去
        //产品的信息数据比较复杂，通过会话技术存储，会话结束数据消失
        //本地存储|会话存储一般存储字符串
